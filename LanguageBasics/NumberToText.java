...
public class NumberToText {
    ...
    public static void  main(String[] args){
        ...
        if (number >= 1000000000) {
        	pomocneNumber = number / 1000000000;
        	if (pomocneNumber < 20) {
        		writeNumber(number / 1000000000);
        		
        	} else if (pomocneNumber >= 20 && pomocneNumber <= 100) {
        		writeNumber(pomocneNumber / 10 * 10);
        		if (pomocneNumber % 10 != 0) {
        			writeNumber(pomocneNumber % 10);
        		}
        	}
        	writeNumber(1000000000);
        	number %= 1000000000;
        }
        
        if (number >= 100000000) {
        	pomocneNumber = number / 1000000;
        	number %= 1000000; 
        	
        	if(pomocneNumber >= 100) {
                writeNumber(pomocneNumber / 100);
                writeNumber(100);
                pomocneNumber %= 100;
            }

            if(pomocneNumber <= 20) {
               writeNumber(pomocneNumber);
            } else if(pomocneNumber < 100) {
                writeNumber(pomocneNumber / 10 * 10);
                if(pomocneNumber % 10 != 0){
                    writeNumber(pomocneNumber % 10);
                }
